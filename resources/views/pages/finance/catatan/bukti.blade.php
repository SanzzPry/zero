        ...
        <div class="bg-white w-[400px] rounded-3xl shadow-2xl p-8 border border-gray-200">
            ...
            <div class="text-sm space-y-3 border-b-1">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ $cash->no_referensi }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">Status</span>
                    <span
                        class="bg-orange-500 text-white px-3 py-1 rounded-md text-xs font-semibold">{{ ucfirst($cash->status) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Jenis Transaksi</span>
                    <span>{{ $cash->jenis }}</span>
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                    ...
                    <span>{{ $cash->sumber_dana }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Tgl/Waktu</span>
                    <span>{{ \Carbon\Carbon::parse($cash->created_at)->translatedFormat('d F Y, H:i') }} WIB</span>
                </div>

                <div class="flex justify-between">
                    <span class="text-gray-600">Nominal</span>
                    <span>Rp {{ number_format($cash->total, 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between pb-3">
                    <span class="text-gray-600">Biaya Admin</span>
                    <span>Rp {{ number_format($cash->biayaadmin, 0, ',', '.')  }}</span>
                </div>
            </div>
            <div class="flex justify-between pt-3 border-t border-dashed border-gray-300 mt-2">
                ...
                <span class="font-semibold">
                    Rp {{ number_format($cash->total + $cash->biayaadmin, 0, ',', '.') }}
                </span>
            </div>
